...
const definitions: NodeDefinition[] = [
  ...
  ['action.email', 'action', 'Send email', 'Send with Resend', 'Mail', z.object({ to: z.string(), subject: z.string(), body: z.string() })],
...
export const WORKFLOW_TEMPLATES: WorkflowDefinition[] = [
  {
    id: 'template-pr-review', name: 'AI Pull Request Reviewer', description: 'Review pull request code changes and post actionable findings', enabled: true, version: 1,
    nodes: [
      ...
      {id:'comment',type:'action.githubComment',name:'Post review',category:'action',position:{x:560,y:120},config:{repository:'{{github.repository}}',issueNumber:'{{github.prNumber}}',body:'{{review.comment}}'}},
    ],
    edges: [
      {id:'e1',source:'github',target:'review'},{id:'e2',source:'review',target:'comment'},
    ],
  },
];
