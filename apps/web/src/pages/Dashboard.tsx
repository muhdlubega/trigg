...
import { Activity, ArrowUpRight, Blocks, Bot, CheckCircle2, Github, Play, Plus, Sparkles } from 'lucide-react';
  ...
  return <div className="page">
    ...
    <div className="dashboard-grid">
      <section className="panel"><header><div><h2>Repository workflows</h2><p>Run a review on the most recently updated open pull request.</p></div><Link to="/workflows">Manage <ArrowUpRight size={14}/></Link></header>
        {isLoading?<Skeleton/>:data?.active.length?<div className="execution-list">{data.active.map((workflow)=><Link to={workflow.repositoryId?`/workflows?repository=${encodeURIComponent(workflow.repositoryId)}`:'/workflows'} key={workflow.id}><span className="event-icon"><Github size={16}/></span><div><b>{workflow.name}</b><small>{workflow.repository}</small></div><button className="button" disabled={run.isPending} onClick={(event)=>{event.preventDefault();run.mutate(workflow.id);}}><Play size={14}/>{run.isPending?'Starting…':'Run now'}</button></Link>)}</div>:<div className="inline-empty"><Sparkles/><p>Connect GitHub and create a repository reviewer to get started.</p></div>}
        {run.error&&<p className="error-text panel-message">{run.error.message}</p>}
        {error&&<p className="error-text panel-message">{error.message}</p>}
      </section>
      <aside className="panel connection"><header><div><h2>Recent executions</h2><p>Latest review activity</p></div><Link to="/executions">View all</Link></header>{data?.recent.length?<div className="execution-list">{data.recent.slice(0,3).map((row)=><Link to={`/executions/${row.id}`} key={String(row.id)}><div><b>{String(row.workflow_name)}</b><small>{new Date(String(row.started_at)).toLocaleString()}</small></div><Status value={String(row.status)}/></Link>)}</div>:<p>No executions yet.</p>}<Link className="button wide" to="/integrations">Manage repositories <ArrowUpRight size={14}/></Link></aside>
    </div>
  </div>;
}
