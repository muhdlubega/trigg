...
export function Workflows(){
  ...
  const repository=repositories.data?.find((repo)=>repo.id===selectedRepositoryId)??repositories.data?.[0];
  ...
  return <div className="page">
    ...
    <div className="workflows-layout">
      <aside className="panel repository-sidebar"><header><div><h2>Repositories</h2><p>{repositories.data?.length??0} available</p></div></header>{repositories.isLoading?<Skeleton rows={5}/>:repositories.data?.length?<div className="repository-list">{repositories.data.map((repo)=><button type="button" className={repository?.id===repo.id?'active':''} key={repo.id} onClick={()=>setSelectedRepositoryId(repo.id)}><Github size={15}/><span><b>{repo.full_name}</b><small>{repo.private?'Private':'Public'}</small></span></button>)}</div>:<div className="repository-empty"><p>No repositories connected.</p><Link className="button" to="/integrations">Connect GitHub</Link></div>}</aside>
