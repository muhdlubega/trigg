...
export function pollWhileActive(status?:string):number|false{return isActiveRun(status)?2000:false;}
export function pollIfAnyActive(rows?:Array<{status?:string|number|null}>):number|false{return rows?.some((row)=>isActiveRun(String(row.status)))?2000:false;}
